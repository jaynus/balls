                ...
                if let Some(queue_ptr) = self.tasks.get(&task_location.into()) {
                    ...
                    if let Some(iter) = iter {
                        ...
                        for entry in &sorted {
                            ...
                            match task.reaction.fetch(&reactions).check(
                                state,
                                Target::Entity(source_entity),
                                Target::Entity(queue_ptr.entity),
                            ) {
                                Ok(_) => {
                                    if let Some(dst) =
    ...
    SystemBuilder::<()>::new("update_task_cache_system")
        ...
        .build(
            move |_command_buffer,
                ...
                //let has_change = task_changed_query.iter_mut(world).next().is_some();
                //if has_change {
                cache.update(
                    task_query
                        .iter_entities(world)
...
impl TaskQueue {
    ...
    pub fn top_any(&self) -> Option<TaskHandle> {
        for set in self.available.values() {
            if let Some(entry) = set.iter().rev().next() {
