...
use rl_core::{
    ...
    components::*,
    data::{SpawnArguments, SpawnPosition, SpawnTarget},
    defs::{
...
pub fn build_sprite_selector(
    ...
    let texture = Arc::new(Texture::from_slice(
        &image::open(path)?
            .as_flat_samples_u8()
            .ok_or(failure::err_msg("Failed to open image"))?
            .as_slice(),
    ...
    UiWindowSet::create_with(
        ...
        move |ui, window_manager, world, resources, _buffer| {
            ...
            imgui::Window::new(im_str!("debugMenu"))
                ...
                .build(ui, || {
                    if imgui::CollapsingHeader::new(ui, im_str!("Save/Load"))
                        .default_open(true)
                        .build()
                    {
                        ui.text("Filler");
                    ...
                    if imgui::CollapsingHeader::new(ui, im_str!("Spawn Item"))
                        .default_open(true)
                        .build()
                    {
                        let materials = {
                                        ...
                                        move |_world, resources, _, coord| {
                                            ...
                                            item_defs
                                                ...
                                                .spawn(
                                                    resources,
                                                    &mut command_buffer,
                                                    SpawnTarget::Position(SpawnPosition::Tile(
                                                        coord,
                                                    )),
                                                    &SpawnArguments::Item {
                    ...
                    if imgui::CollapsingHeader::new(ui, im_str!("Spawn Workshop"))
                        .default_open(true)
                        .build()
                    {
                        let workshops = {
                                    ...
                                    rl_core::input::PlacementRequestImpl::new(
                                        ...
                                        move |_world, resources, _, coord| {
                                            ...
                                            let _ = def_storage.get_by_name(name).unwrap().spawn(
                                                resources,
                                                &mut command_buffer,
                                                SpawnTarget::Position(SpawnPosition::Tile(coord)),
        ...
        move |ui, _window_manager, world, resources, _buffer| {
            imgui::Window::new(im_str!("debugOverlay"))
                ...
                .build(ui, || {
                    ...
                    if imgui::CollapsingHeader::new(ui, im_str!("Weather"))
                        .default_open(true)
                        .build()
                    {
                        imgui::Slider::new(
                    ...
                    if imgui::CollapsingHeader::new(ui, im_str!("Selection"))
                        .default_open(true)
                        .build()
                    {
                            ...
                            if !selection.entities.is_empty() {
                                ...
                                for entity in &selection.entities {
                                        ...
                                        world.get_component::<MaterialComponent>(*entity)
                                    {
                                        ui.text(&format!(
                                            "{}",
                                            material_comp.fetch_state(&material_defs).name()
                                        ));
                                    }
                                    ui.next_column();
                    ...
                    if imgui::CollapsingHeader::new(ui, im_str!("Pawns"))
                        .default_open(true)
                        .build()
                    {
                        let query = <(
                    ...
                    if imgui::CollapsingHeader::new(ui, im_str!("Tile Inspector"))
                        .default_open(true)
                        .build()
                    {
