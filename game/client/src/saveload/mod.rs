...
macro_rules! component_registration {
    ( $( $t:ident, )* ) => (
        [
            $(
                ComponentRegistration::of::<$t>(),
            )*
        ]
    )
}

macro_rules! tag_registration {
    ( $( $t:ident, )* ) => (
        [
            $(
                TagRegistration::of::<$t>(),
            )*
        ]
    )
}

pub fn serialize_world(world: &World) -> Result<String, failure::Error> {
    use rl_core::{components::*, ron};

    let comp_registrations = component_registration!(
        PositionComponent,
        DimensionsComponent,
        ItemComponent,
        ...
        NeedsComponent,
        //ActivePickupComponent, //Dont need this
        NameComponent,
        Destroy,
        EntityMeta,
    );

    let tag_registrations = tag_registration!(
        PawnTag,
        StaticTag,
        BuildingTag,
        WorkshopTag,
    ...
    let entity_map = Arc::new(RefCell::new(HashMap::new()));
    rl_core::saveload::entity::SER_ENTITY_MAP.with(|map| *map.borrow_mut() = entity_map.clone());
    ...
    for ComponentSeqDeserializer<'a, T>
{
        ...
        A: de::SeqAccess<'de>,
    {
        ...
        for _ in 0..size.unwrap_or(usize::max_value()) {
...
type ComponentDeserializeFn = fn(
    deserializer: &mut dyn erased_serde::Deserializer,
...
struct SerializeImpl {
    ...
    entity_map: Arc<RefCell<HashMap<Entity, type_uuid::Bytes>>>,
}
impl legion::serialize::ser::WorldSerializer for SerializeImpl {
    ...
    fn serialize_archetype_description<S: Serializer>(
        ...
        let tags_to_serialize = archetype_desc
            .tags()
            .iter()
            .filter_map(|(ty, _)| self.tag_types.get(&ty.0).map(|r| r.uuid))
            .collect::<Vec<_>>();

        let components_to_serialize = archetype_desc
            .components()
            .iter()
            .filter_map(|(ty, _)| self.comp_types.get(&ty.0).map(|r| r.uuid))
            .collect::<Vec<_>>();
...
struct DeserializeImpl {
    ...
    entity_map: Arc<RefCell<HashMap<uuid::Bytes, Entity>>>,
}
