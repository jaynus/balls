...
pub struct LoadWorldEvent {
    closure: Arc<dyn Fn(&mut World, &mut Resources) -> Result<(), anyhow::Error> + Send + Sync>,
}
impl LoadWorldEvent {
    pub fn new<F>(f: F) -> Self
    where
        F: 'static + Fn(&mut World, &mut Resources) -> Result<(), anyhow::Error> + Send + Sync,
    {
        Self {
            closure: Arc::new(f),
        }
...
impl Application {
    ...
    pub fn run(mut self, event_loop: EventLoop<()>) -> Result<(), anyhow::Error> {
        let mut exit = false;
    ...
    pub fn on_event(&mut self, event: &Event<()>) -> Result<(), anyhow::Error> {
    ...
    pub fn on_tick(&mut self) -> Result<(), anyhow::Error> {
        for manager in &mut self.managers {
