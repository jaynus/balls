...
use crate::{
    bit_set::BitSet, derivative::Derivative, fxhash::FxHashMap, legion::prelude::*, ron, serde,
};
use std::{
    convert::TryInto,
    path::{Path, PathBuf},
...
pub trait DefinitionResolver<T> {
    fn resolve(def: &mut T, resources: &Resources) -> Result<(), anyhow::Error>;
}
...
impl<T> DefinitionResolver<T> for DefaultDefinitionResolver<T> {
    fn resolve(_def: &mut T, _resources: &Resources) -> Result<(), anyhow::Error> {
        Ok(())
    }
}
        ...
        P: AsRef<Path>,
    {
        ...
        for entry in files {
            let contents = std::fs::read_to_string(&entry)
                .map_err(|e| anyhow::anyhow!("[{}]: {}", entry.as_path().display(), e))?;
    ...
    T: std::fmt::Debug + Definition + for<'a> serde::Deserialize<'a>,
{
    ...
    pub fn resolve(&mut self, resources: &mut Resources) -> Result<(), anyhow::Error> {
        for def in self.iter_mut() {
            T::Resolver::resolve(
                Arc::get_mut(def)
                    .ok_or_else(|| anyhow::anyhow!("Failed to get mutable Arc for definition"))?,
                resources,
            )?;
        }
        Ok(())
    ...
    pub fn reload(resources: &mut Resources) -> Result<(), anyhow::Error> {
        T::Loader::reload(resources)
    }

    pub fn from_folder<P>(resources: &mut Resources, folder: P) -> Result<(), anyhow::Error>
    where
        P: AsRef<Path>,
    {
...
pub fn load_all_defs(resources: &mut Resources, root: Option<&str>) -> Result<(), anyhow::Error> {
