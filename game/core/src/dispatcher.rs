...
pub struct DispatcherSystemBundle<F>(F);
impl<F> ConsumeDesc for DispatcherSystemBundle<F>
where
    F: FnOnce(&mut World, &mut Resources, &mut DispatcherBuilder) -> Result<(), failure::Error>,
{
    fn consume(
        self: Box<Self>,
        world: &mut World,
...
impl DispatcherData {
    pub fn flatten(self) -> Result<Dispatcher, failure::Error> {
...
impl<'a> DispatcherBuilder<'a> {
    ...
    pub fn build(
        ...
        world: &mut World,
        resources: &mut Resources,
    ) -> Result<Dispatcher, failure::Error> {
