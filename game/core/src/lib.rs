...
impl<T: Copy + Eq + std::hash::Hash> FreeList<T> {
    ...
    pub fn push(&mut self, value: T) -> Result<(), anyhow::Error> {
        if !self.consumed.remove(&value) {
            return Err(anyhow::anyhow!("Item returned not already in the consumed list. This belongs to a different FreeList"));
        }
