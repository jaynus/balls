...
pub fn definition_derive(input: TokenStream) -> TokenStream {
    ...
    let mut gen = quote! {
        ...
        impl #ref_type_name {
            ...
            pub fn resolve(&mut self, storage: &crate::defs::DefinitionStorage<#name>) -> Result<(), crate::failure::Error> {
                self.id = Some(storage.get_id(&self.name).ok_or(crate::failure::format_err!("Invalid definition ref: type={}, {}", std::any::type_name::<#name>(), self.name))?);
