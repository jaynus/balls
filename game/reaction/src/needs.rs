...
impl ReactionEffect for ConsumeEdibleEffect {
    ...
    fn tick(
        ...
        for (_reagent_entry, target) in entities.iter() {
            let nutrition = get_nutrition(state, target.entity().unwrap()).unwrap();
