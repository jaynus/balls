...
use rl_core::{app, failure, winit::event_loop::EventLoop, GameState, Manager};

type PassConstructor =
    Box<dyn FnOnce(&mut RenderArgs<'_>) -> Result<Box<dyn GlPass>, failure::Error>>;
...
impl RenderManagerBuilder {
    ...
    where
        F: 'static + FnOnce(&mut RenderArgs<'_>) -> Result<Box<dyn GlPass>, failure::Error>,
    {
    ...
    ) -> Result<RenderManager, failure::Error> {
