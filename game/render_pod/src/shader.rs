...
impl ShaderSource {
    ...
    pub fn from_src_path<P>(kind: ShaderKind, path: P) -> Result<Self, anyhow::Error>
    where
        P: AsRef<Path>,
    {
        Self::from_src_reader(
            ...
            path.as_ref().to_str().ok_or_else(|| {
                anyhow::anyhow!("Failed to parse provided path for shader compiler")
            })?,
...
impl ShaderSourceSet {
    pub fn from(shaders: &[ShaderSource]) -> Result<Self, anyhow::Error> {
        ...
        for shader in shaders.iter() {
            ...
            if stage.contains(ReflectShaderStageFlags::VERTEX) {
                if set.vertex.is_none() {
                    ...
                    return Err(anyhow::anyhow!("Duplicate shader stage in set"));
                }
