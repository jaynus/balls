...
use rl_core::{failure, legion::prelude::*, winit::window::Window, ScreenDimensions};
use rl_render_glow::{
...
impl ImguiPass {
    pub fn new(args: &mut RenderArgs<'_>) -> Result<Box<dyn GlPass>, failure::Error> {
...
impl GlPass for ImguiPass {
    unsafe fn draw(&mut self, args: RenderArgs<'_>) -> Result<(), failure::Error> {
        let gl = &args.render_context.gl;
