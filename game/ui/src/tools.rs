        ...
        move |ui, _window_manager, world, resources, command_buffer| {
            ...
            if got_event {
                if let Some(action) = &state.active_designation {
                    let (selection_state, map, spatial_map, reaction_defs) =
                        <(
                            Read<SelectionState>,
                            Read<Map>,
                            Read<SpatialMap>,
...
pub fn spawn_tasks_on_entity(
    ...
    selection_area.for_each(|coord| {
        let tile = map.get(coord);
        // TODO: can designate should return target entities?
        if let Ok(Some(mut entities)) =
            def.can_designate(GameStateRef { world, resources }, map, coord, tile)
        {
            entities.drain(..).for_each(|entity| {
                if let Some(tasks) = world.get_component::<HasTasksComponent>(entity) {
                    ...
                    command_buffer.add_component(
                        entity,
                        HasTasksComponent::from_iter(vec![Task::new(
                            5,
                            TaskKind::MINING,
                            def.id(),
                        )]),
...
pub fn spawn_virtual_tasks(
    ...
    selection_area.for_each(|coord| {
        let tile = map.get(coord);
        if def
            .can_designate(GameStateRef { world, resources }, map, coord, tile)
            .is_ok()
        {
            use std::iter::FromIterator;
